...
def init_db() -> None:
    with closing(get_db()) as conn:
        # Journal mode is persistent and may require an exclusive lock. Set it once
        # during startup instead of blocking every web request and monitor check.
...
def _ensure_monitor_columns(cursor: sqlite3.Cursor) -> None:
    ...
    required_columns = {
        ...
        "ping_mode": "TEXT NOT NULL DEFAULT 'or'",
        "ping_target": "TEXT",
        "last_error_category": "TEXT",
        "is_flapping": "INTEGER NOT NULL DEFAULT 0",
        "flapping_until": "TEXT",
    ...
    with closing(get_db()) as conn:
        ...
        cursor.execute(
            ...
            (
                ...
                target.strip(),
                int(ping_enabled),
                "and" if str(ping_mode).lower() == "and" else "or",
        ...
        for monitor in monitors:
            cursor.execute(
                ...
                (
                    ...
                    "and" if str(monitor.get("ping_mode", "or")).lower() == "and" else "or",
